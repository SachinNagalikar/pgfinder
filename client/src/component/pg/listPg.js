...
class PgList extends React.Component {
  ...
  filterPGs(change) {
    ...
    if (change.roomTypes.singleSharing.value) {
      localPg.singleShare = pg.filter(x =>
        x.roomTypes.includes(change.roomTypes.singleSharing.name)
      );
      hasFilter = true;
    }
    if (change.roomTypes.twoSharing.value) {
      localPg.twoSharing = pg.filter(x =>
        x.roomTypes.includes(change.roomTypes.twoSharing.name)
      );
      hasFilter = true;
    }

    if (change.roomTypes.threeSharing.value) {
      localPg.threeSharing = pg.filter(x =>
        x.roomTypes.includes(change.roomTypes.threeSharing.name)
      );
      hasFilter = true;
    }
    if (change.roomTypes.fourSharing.value) {
      localPg.fourSharing = pg.filter(x =>
        x.roomTypes.includes(change.roomTypes.fourSharing.name)
      );
      hasFilter = true;
    }
    if (hasFilter) {
      pg = this.findUnique(
        localPg.singleShare
          .concat(localPg.twoSharing)
          .concat(localPg.threeSharing)
          .concat(localPg.fourSharing),
        d => d._id
      );
    }
    return pg;
      ...
      <React.Fragment>
                      ...
                      <Grid item key={pg._id} sm={3} md={3} lg={4}>
                        <Card className={classes.card}>
                          ...
                          <CardContent className={classes.cardContent}>
                            ...
                            <Typography>{`PG Type:-${pg.pgTypes}`}</Typography>
                            <Typography>
                          ...
                          <CardActions>
                            <Button size="small" color="primary">
                              <Link to={`/pg/${pg._id}`}>view</Link>{" "}
                            </Button>{" "}
                            <Button size="small" color="primary">
                              <Link to={`/pg/edit/${pg._id}`}>edit</Link>
                            </Button>
                          </CardActions>
                        </Card>
                        {/* {isOpen && ( 
        ...
        <footer className={classes.footer}>
          ...
          <Typography
            ...
            component="p"
          >
            Developed By Kumar sundram and Sachin Nagalikar
          </Typography>
        </footer>
      </React.Fragment>
    );
